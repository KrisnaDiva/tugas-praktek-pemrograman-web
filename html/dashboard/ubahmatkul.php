...
session_start();
$semester = $_GET['x'];
$sql = "SELECT * FROM tb_matkul WHERE semester = '$semester'";
$aksi = mysqli_query($koneksi, $sql);
$isi = mysqli_fetch_all($aksi, MYSQLI_ASSOC);
$sqll = "SELECT COUNT(*) AS total FROM tb_matkul WHERE semester = '$semester'";
$aksii = mysqli_query($koneksi, $sqll);
$isii = mysqli_fetch_assoc($aksii);
...
if (isset($_POST['submit'])) {
    ...
    // Melakukan proses update ke dalam tabel matakuliah (disesuaikan dengan konfigurasi database Anda)
    ...
    // Menyimpan data dari input fields ke dalam array
    $inputs = array();
    for ($i = 0; $i < $quantity; $i++) {
        $inputValues = array();
        for ($j = 0; $j < 4; $j++) {
            $inputName = "input" . ($i * 4 + $j);
    ...
    foreach ($inputs as $inputValues) {
        $kmk = $inputValues[0];
        $namamatkul = $inputValues[1];
        $sks = $inputValues[2];
        $semesterr = $inputValues[3];
        $sql = "UPDATE tb_matkul SET namamatkul='$namamatkul', sks='$sks',semester='$semesterr' WHERE kmk='$kmk'";
        $conn->query($sql);
    }
...
<body class="  ">
  ...
  <main class="main-content">
    ...
    <form method="POST" action="" class="">
        ...
        <label for="quantity">Jumlah Data:</label>
        <input type="number" id="quantity" name="quantity" min="1" max="100" value="<?php echo count($isi); ?>">
        <br><br>
  ...
  <script>
        var data = <?php echo json_encode($isi); ?>;
        ...
        function generateInputs(quantity) {
            var inputContainer = document.getElementById("inputContainer");
            inputContainer.innerHTML = "";

            var headerRow = document.createElement("div");
            headerRow.classList.add("input-container", "header-row");

            var header1 = document.createElement("span");
            header1.textContent = "kmk";
            var header2 = document.createElement("span");
            header2.textContent = "namamatkul";
            var header3 = document.createElement("span");
            header3.textContent = "sks";
            var header4 = document.createElement("span");
            ...
            for (var i = 0; i < quantity; i++) {
                ...
                inputRow.classList.add("input-container");

                for (var j = 0; j < 4; j++) {
                    var input = document.createElement("input");
                    input.type = "text";
                    input.name = "input" + (i * 4 + j);
                    input.value = data[i][Object.keys(data[i])[j]]; // Mengisi nilai input dengan data yang sudah ada
