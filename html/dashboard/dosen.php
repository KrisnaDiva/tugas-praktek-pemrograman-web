...
if (isset($_POST['status'])) {
  $status = $_POST['status'];
  $_SESSION['status'] = $status; // Simpan status dalam session
  $halaman = 1;
} elseif (isset($_SESSION['status'])) {
  $status = $_SESSION['status']; // Gunakan status dari session jika sudah disimpan sebelumnya
} else {
  $status = 'all';
}
      ...
      <nav class="nav navbar navbar-expand-lg navbar-light iq-navbar">
        <div class="container-fluid navbar-inner">
          ...
          <div class="input-group search-input">
              <label style="font-size: 20px;">Dosen / <h4 style="font-weight: 700;display: inline-block;">Data</h4></label>
            </div>
              ...
              <form action="" method="post">
                <div class="row align-items-center">
                  <div class="col-md-6 col-sm-12 text-lg-start text-md-start">
                    ...
                    <select id="status" class="form-select" name="status" onchange="this.form.submit()" style="width: auto;display:inline;">
                      <option value="all" <?php if ($status == 'all') echo 'selected'; ?>>Semua status</option>
                      <option value="Dosen Tetap" <?php if ($status == 'Dosen Tetap') echo 'selected'; ?>>Dosen Tetap</option>
                      <option value="Dosen Tidak Tetap" <?php if ($status == 'Dosen Tidak Tetap') echo 'selected'; ?>>Dosen Tidak Tetap</option>
                      <option value="Dosen Honorer" <?php if ($status == 'Dosen Honorer') echo 'selected'; ?>>Dosen Honorer
                    </select>
                  </div>
                  <div class="col-md-6 col-sm-12 text-lg-end text-md-start">
                    <label for="nidn">Cari : </label>
                    <input type="text" class="form-control form-control-sm" id="nidn" name="nidn" placeholder="Masukkan NIDN atau Nama" style="width: auto;display:inline;">

                    <button type="submit" class="btn btn-primary btn-auto"><i class=" fa fa-search"></i></button>
                    <a type="button" class="btn btn-danger btn-auto" href="dosen.php"><i class=" fa fa-x"></i></a>
              ...
              <div class="table-responsive">
                <table id="datatables" class="table table-striped table-bordered" data-toggle="table-ascdesc">
                  ...
                  <tbody>
                    ...
                    if (!empty($nidn)) {
                      $where = "WHERE nidn like '%".$nidn."%' or nama like '%".$nidn."%'";
                    }
                    $batas = 10;
                    ...
                    $proses = mysqli_query($koneksi, "SELECT * FROM dosen $where LIMIT $halaman_awal, $batas");
                    ...
                    ?>
                      <tr>
                        ...
                        <td>
                          <a type="button" class="btn btn-primary" href="ubahdosen.php?x=<?php echo $isi['nidn'] ?>"><i class='fa fa-edit'></i></a>
                          <a type="button" class="btn btn-danger" href="hapusdosen.php?x=<?php echo $isi['nidn']; ?>"><i class='fa fa-trash'></i></a>
                        </td>
                      </tr>
                    <?php
                    endwhile;
                ...
                <a type="button" class="btn btn-success" href="tambahdosen.php">Tambah Dosen</a>
              </div>
            </div>
          </div>
        </div>
