...
if ($.fn.DataTable) {
  if ($('[data-toggle="table-ascdesc"]').length) {
    const table = $('[data-toggle="table-ascdesc"]').DataTable({
      "dom": '<"row align-items-center"<"col-md-6" l><"col-md-6" f>><"table-responsive border-bottom my-3" rt><"row align-items-center" <"col-md-6" i><"col-md-6" p>><"clear">',
    });

    // Mengatur data toggle untuk kolom NIM
    $('[data-toggle="table-ascdesc"] th:nth-child(2)').attr('data-order', 'asc');
    $('[data-toggle="table-ascdesc"] th:nth-child(2)').on('click', function() {
      table.order([1, $(this).data('order')]).draw();
      $(this).data('order', $(this).data('order') === 'asc' ? 'desc' : 'asc');
    });

    // Mengatur data toggle untuk kolom Nama
    $('[data-toggle="table-ascdesc"] th:nth-child(3)').attr('data-order', 'asc');
    $('[data-toggle="table-ascdesc"] th:nth-child(3)').on('click', function() {
      table.order([2, $(this).data('order')]).draw();
      $(this).data('order', $(this).data('order') === 'asc' ? 'desc' : 'asc');
    });
  }
